...
- Периодические ходы врага (сложность Easy/Average/Difficult + **туманность миссии** для темпа AI).
...
- Сложность: `ticksPerEnemyTurn` / divider туманности + параметры `EnemyPersonality` из `GameDifficulty`.
...
## 7. Карта кампании (туманности по 30)

### 7.1 Контент миссии (`Scene`)

| Поле | Назначение |
|------|------------|
| `id` | 1…N (v1: 40; целевой горизонт ~300) |
...
- **Туманности по 30 миссий**, плитка **5 колонок × до 6 рядов** на туманность.
- Неполные туманности допустимы (по мере появления контента).
- Горизонтальный **свайп** между туманностями; при входе на карту — **текущая**.
- **Без линий** между миссиями: пройденные / текущая / закрытые узлы.
- Имена (RU/EN): Орион, Андромеда, Конская Голова, Краб, Орёл, Лагуна, Вуаль, Карина, Тарантул, Дева.
- Темп AI в бою — **ступенька по туманности миссии** (`CampaignSectors`), plateau внутри туманности, cap ×2.5.
...
- у врага дополнительно «частота хода AI» — **ступенька по туманности миссии** (см. §7.2), не покупка игроком.

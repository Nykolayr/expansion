...
- [x] Настройки: повтор истории (`/intro-story`), выбор ru/en
...
- [x] Поле 5×8, свайп отправки, тактическая панель
- [x] Победа / поражение → диалог → карта, `mapClassic++`, `firstBattleCompleted`
- [x] Анимация флотов, астероиды, тактические апгрейды
- [x] Isolate game loop, спрайты боя, меню паузы
- [x] Туториал миссии 1, справка `/help`, playtest-checklist
- [ ] Баланс playtest (итерации по чеклисту)

### Фаза 4 — Begin + прогресс — ✅

- [x] Экран «Новая игра», Progress, мета-апгрейды
- [x] Профиль гостя (имя, статистика)
- [x] Автоскролл карты, пост-победа (карта / улучшения / следующая миссия)

### Фаза 5 — Аккаунт и сервер — MVP 🚧

- [x] `expansion_server` MVP: health, auth, profile, content/version (in-memory)
- [ ] Profile Login UI + синхронизация клиента
- [ ] Update: докачка сцен в SQLite

### Фаза 6 — Полировка и релиз — частично

- [x] Звук (сервис + настройка; ассеты — по мере переноса)
- [x] Help, донат (ссылки)
- [x] Haptic feedback в бою
- [ ] Релиз AAB (чеклисты `.cursor/`)
...
> **Актуальный приоритет (2026-07-11):** см. **[`docs/ROADMAP.md`](../../docs/ROADMAP.md)** — не кампания, а платформа.

| # | Фокус | Статус |
|---|--------|--------|
| **1** | **Сервер:** регистрация, авторизация, БД пользователей | Следующий — ждём видение и вопросы |
| **2** | **Арты:** промпты на каждый объект, картинки — пользователем | После п.1 |
| **3** | **Кампания:** м6+ по [`campaign-missions-1-15.md`](campaign-missions-1-15.md) | Отложено |

Клиент: бой м1–5, карта, мета-апгрейды, гость — **сделано**. М6 «Альциона» (`shielded`) — в очереди пункта 3.
